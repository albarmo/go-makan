...
export default function StoresPage(props: { children?: any }) {
  return (
    <RoleGuard>
      <StoresContent>{props.children}</StoresContent>
    </RoleGuard>
  );
}

function StoresContent(props: { children?: any }) {
  const location = useLocation();
  const stores = createAsync(() => getStores());
  const [search, setSearch] = createSignal("");

  const isIndexRoute = () =>
    location.pathname === "/stores" || location.pathname === "/stores/";
  ...
  return (
    <>
      ...
      <Show when={isIndexRoute()} fallback={props.children}>
                  ...
                  <For each={filteredStores()}>
                    {(store) => (
                      <A
                        ...
                        class="tm-card block overflow-hidden"
                      >
